...
public class Scontroller implements Initializable, Observer, Observable {
	...
	@FXML
	private Text soCarrier;
	@FXML
	private Text soBattleship;
	@FXML
	private Text soCruiser;
	@FXML
	private Text soSubmarine;
	@FXML
	private Text soDestroyer;
	@FXML
	private Text spCarrier;
	@FXML
	private Text spBattleship;
	@FXML
	private Text spCruiser;
	@FXML
	private Text spSubmarine;
	@FXML
	private Text spDestroyer;
	
	@FXML
	private Button sAutoShips;

	@FXML
	private ChoiceBox<String> OrientationChoice;
	private String orientation = "Horizontal";

	private Armada serverArmada = new Armada();
	...
	public void initialize(URL location, ResourceBundle resources) {
		buttonList = SplayerGrid.getChildren();
		for (Node node : SopponentGrid.getChildren()) {
			if (node.getId() != null) {
				// System.out.println(node.getId());
				this.ServerInitMouseEvent((Button) node);
			}
		}
		...
		sAutoShips.setStyle("-fx-background-color: white");
		...
		sAutoShips.addEventHandler(javafx.scene.input.MouseEvent.MOUSE_CLICKED, automateArmada);
	...
	public void ServerInitMouseEvent(Button b) {
		b.addEventHandler(javafx.scene.input.MouseEvent.MOUSE_CLICKED, serverMouseClickEvent);
	}
	...
	public void setServerOutputStream(DataOutputStream s) {
		sOut = s;
	}

	public void setServerInputStream(DataInputStream s) {
		sIn = s;
	}
	...
	private EventHandler<MouseEvent> serverMouseClickEvent = new EventHandler<MouseEvent>() {

		@Override
		public void handle(MouseEvent e) {
			System.out.println("CONTROLLER SERVER: Turn is: " + turn);

			if (turn == serverTurn && isShipsSet) {
				char client1Add = 'X';
				toSend = ((Node) e.getTarget()).getId();
				toSend = new StringBuilder(toSend).append(client1Add).toString();
	...
	public void updateSplayerGrid(String pos, String HM) {
		for (Node node : SplayerGrid.getChildren()) {
			if (node.getId() != null) {
				if (node.getId().equals(pos)) {
					if (HM.equals("Hit")) {
						...
						node.setStyle("-fx-background-color: red");
						if (!CarrierSunk && serverArmada.isCarrierSunk()) {
							CarrierSunk = true;
							if(CarrierSunk && BattleshipSunk && CruiserSunk && 
									SubmarineSunk && DestroyerSunk) {
								System.out.println("CARRIER SUNK");
								spCarrier.setStyle("-fx-text-fill: red;");
								spCarrier.setStyle("-fx-strikethrough: true");
								notifyObserver("OVER");
							}
							else {
							CarrierSunk = true;
						...
						if (!BattleshipSunk && serverArmada.isBattleshipSunk()) {
							BattleshipSunk = true;
							if(CarrierSunk && BattleshipSunk && CruiserSunk && 
									SubmarineSunk && DestroyerSunk) {
								System.out.println("BATTLESHIP SUNK");
								spBattleship.setStyle("-fx-text-fill: red;");
								spBattleship.setStyle("-fx-strikethrough: true");
								notifyObserver("OVER");
							}
							else {
							System.out.println("BATTLESHIP SUNK");
						...
						if (!CruiserSunk && serverArmada.isCruiserSunk()) {
							CruiserSunk = true;
							if(CarrierSunk && BattleshipSunk && CruiserSunk && 
									SubmarineSunk && DestroyerSunk) {
								System.out.println("CRUISER SUNK");
								spCruiser.setStyle("-fx-text-fill: red;");
								spCruiser.setStyle("-fx-strikethrough: true");
								notifyObserver("OVER");
							}
							else {
							System.out.println("CRUISER SUNK");
						...
						if (!SubmarineSunk && serverArmada.isSubmarineSunk()) {
							SubmarineSunk = true;
							if(CarrierSunk && BattleshipSunk && CruiserSunk && 
									SubmarineSunk && DestroyerSunk) {
								System.out.println("SUBMARINE SUNK");
								spSubmarine.setStyle("-fx-text-fill: red;");
								spSubmarine.setStyle("-fx-strikethrough: true");
								notifyObserver("OVER");
							}
							else {
							System.out.println("SUBMARINE SUNK");
						...
						if (!DestroyerSunk && serverArmada.isDestroyerSunk()) {
							DestroyerSunk = true;
							if(CarrierSunk && BattleshipSunk && CruiserSunk && 
									SubmarineSunk && DestroyerSunk) {
								System.out.println("DESTROYER SUNK");
								spDestroyer.setStyle("-fx-text-fill: red;");
								spDestroyer.setStyle("-fx-strikethrough: true");
								notifyObserver("OVER");
							}
							else {
							System.out.println("DESTROYER SUNK");
	...
	public void update(String s) {
		System.out.println("SC: Received " + s + " from Server.");

		if (s.equals("SET")) {
			isShipsSet = true;
		}
		else if(s.equals("CARRIER")) {
			soCarrier.setStyle("-fx-text-fill: red;");
			soCarrier.setStyle("-fx-strikethrough: true");
		}
		else if(s.equals("BATTLESHIP")) {
			soBattleship.setStyle("-fx-text-fill: red;");
			soBattleship.setStyle("-fx-strikethrough: true");
		}
		else if(s.equals("CRUISER")) {
			soCruiser.setStyle("-fx-text-fill: red;");
			soCruiser.setStyle("-fx-strikethrough: true");
		}
		else if(s.equals("SUBMARINE")) {
			soSubmarine.setStyle("-fx-text-fill: red;");
			soSubmarine.setStyle("-fx-strikethrough: true");
		}
		else if(s.equals("DESTROYER")) {
			soDestroyer.setStyle("-fx-text-fill: red;");
			soDestroyer.setStyle("-fx-strikethrough: true");
		}
		else if (turn == 2 && myTurn) {
			...
			/*
			 * if(s.length() == 4) { if(s.charAt(3) == '1') turn = 1; } else if(s.length()
			 * == 5) { if(s.charAt(4) == '1') turn = 1; }
			 */
			...
			boolean isHit = serverArmada.calculateHit(t);
			serverArmada.updateArmada(t);
			...
			updateSplayerGrid(t, HorM);

			notifyObserver(HorM + "2");
			turn = 2;
			myTurn = false;
		}
...
//					System.out.println("Vertical");
		...
		for(int i = 0; i < battleship.size(); i++) {
			serverArmada.addToBattleship(battleship.get(i));
		}
			...
			for (int j = 0; j < 4; j++) {
				if (buttonList.get(k).getId() != null) {
					if (buttonList.get(k).getId().toString().length() == 3) {
						if (buttonList.get(k).getId().toString().charAt(1) == serverArmada.getBattleShip().get(j).charAt(0)
								&& buttonList.get(k).getId().toString().charAt(2) == serverArmada.getBattleShip().get(j)
										.charAt(1)
								&& serverArmada.getBattleShip().get(j).length() == 2) {
						...
						if (buttonList.get(k).getId().toString().charAt(1) == serverArmada.getBattleShip().get(j).charAt(0)
								&& buttonList.get(k).getId().toString().charAt(2) == serverArmada.getBattleShip().get(j)
										.charAt(1)
										&& serverArmada.getBattleShip().get(j).length() == 3
										&& buttonList.get(k).getId().toString().charAt(3) == serverArmada.getBattleShip().get(j)
										.charAt(2)) {
							buttonList.get(k).setStyle("-fx-background-color: green");
							buttonList.get(k).setDisable(true);
						}
...
//					System.out.println("Vertical");
		...
		for(int i = 0; i < cruiser.size(); i++) {
			serverArmada.addToCruiser(cruiser.get(i));
		}
		...
		for (int k = 0; k < buttonList.size(); k++) {
			for (int j = 0; j < 3; j++) {
				if (buttonList.get(k).getId() != null) {
					if (buttonList.get(k).getId().toString().length() == 3) {
						if ( buttonList.get(k).getId().toString().charAt(1) == serverArmada.getCruiser().get(j).charAt(0)
								&& buttonList.get(k).getId().toString().charAt(2) == serverArmada.getCruiser().get(j)
										.charAt(1) && serverArmada.getCruiser().get(j).length() == 2
								) {
							buttonList.get(k).setDisable(true);
							buttonList.get(k).setStyle("-fx-background-color: green");
						}
						...
						if (buttonList.get(k).getId().toString().charAt(1) == serverArmada.getCruiser().get(j).charAt(0)
								&& buttonList.get(k).getId().toString().charAt(2) == serverArmada.getCruiser().get(j)
										.charAt(1)
										&& serverArmada.getCruiser().get(j).length() == 3
										&& buttonList.get(k).getId().toString().charAt(3) == serverArmada.getCruiser().get(j)
										.charAt(2)) {
							buttonList.get(k).setDisable(true);
							buttonList.get(k).setStyle("-fx-background-color: green");
						}
...
//					System.out.println("Vertical");
		...
		for(int i = 0; i < submarine.size(); i++) {
			serverArmada.addToSubmarine(submarine.get(i));
		}
		...
		for (int k = 0; k < buttonList.size(); k++) {
			for (int j = 0; j < 3; j++) {
				if (buttonList.get(k).getId() != null) {
					if (buttonList.get(k).getId().toString().length() == 3) {
						if ( buttonList.get(k).getId().toString().charAt(1) == serverArmada.getSubmarine().get(j).charAt(0)
								&& buttonList.get(k).getId().toString().charAt(2) == serverArmada.getSubmarine().get(j)
										.charAt(1) && serverArmada.getSubmarine().get(j).length() == 2
								) {
							buttonList.get(k).setStyle("-fx-background-color: green");
							buttonList.get(k).setDisable(true);
						}
						...
						if (buttonList.get(k).getId().toString().charAt(1) == serverArmada.getSubmarine().get(j).charAt(0)
								&& buttonList.get(k).getId().toString().charAt(2) == serverArmada.getSubmarine().get(j)
										.charAt(1)
										&& serverArmada.getSubmarine().get(j).length() == 3
										&& buttonList.get(k).getId().toString().charAt(3) == serverArmada.getSubmarine().get(j)
										.charAt(2)) {
							buttonList.get(k).setStyle("-fx-background-color: green");
							buttonList.get(k).setDisable(true);
...
//					System.out.println("Vertical");
		...
		for(int i = 0; i < destroyer.size(); i++) {
			serverArmada.addToDestroyer(destroyer.get(i));
		}
		...
		for (int k = 0; k < buttonList.size(); k++) {
			for (int j = 0; j < 2; j++) {
				if (buttonList.get(k).getId() != null) {
					if (buttonList.get(k).getId().toString().length() == 3) {
						if ( buttonList.get(k).getId().toString().charAt(1) == serverArmada.getDestroyer().get(j).charAt(0)
								&& buttonList.get(k).getId().toString().charAt(2) == serverArmada.getDestroyer().get(j)
										.charAt(1) && serverArmada.getDestroyer().get(j).length() == 2
								) {
							buttonList.get(k).setDisable(true);
							buttonList.get(k).setStyle("-fx-background-color: green");
						}
						...
						if (buttonList.get(k).getId().toString().charAt(1) == serverArmada.getDestroyer().get(j).charAt(0)
								&& buttonList.get(k).getId().toString().charAt(2) == serverArmada.getDestroyer().get(j)
										.charAt(1)
										&& serverArmada.getDestroyer().get(j).length() == 3
										&& buttonList.get(k).getId().toString().charAt(3) == serverArmada.getDestroyer().get(j)
										.charAt(2)) {
							buttonList.get(k).setDisable(true);
							buttonList.get(k).setStyle("-fx-background-color: green");
						}
...
//				System.out.println("Horizontal");
				...
				if (placeableButton == true) {
					serverArmada.addToCarrier(buttonToInsert);
					used.add(buttonToInsert);

					if (buttonToInsert.charAt(0) == 'C') {
						sb.setCharAt(0, 'A');
						serverArmada.addToCarrier(sb.toString());
...
//				System.out.println("Vertical");
		...
		for (int k = 0; k < buttonList.size(); k++) {
			for (int j = 0; j < 5; j++) {
				if (buttonList.get(k).getId() != null) {
					if (buttonList.get(k).getId().toString().length() == 3) {
						if (buttonList.get(k).getId().toString().charAt(1) == serverArmada.getCarrier().get(j).charAt(0)
								&& buttonList.get(k).getId().toString().charAt(2) == serverArmada.getCarrier().get(j)
										.charAt(1)
								&& serverArmada.getCarrier().get(j).length() == 2) {
							buttonList.get(k).setDisable(true);
							buttonList.get(k).setStyle("-fx-background-color: green");
						}
					} else if (buttonList.get(k).getId().toString().length() == 4) {
						if (buttonList.get(k).getId().toString().charAt(1) == serverArmada.getCarrier().get(j).charAt(0)
								&& buttonList.get(k).getId().toString().charAt(2) == serverArmada.getCarrier().get(j)
										.charAt(1)
										&& serverArmada.getCarrier().get(j).length() == 3
										&& buttonList.get(k).getId().toString().charAt(3) == serverArmada.getCarrier().get(j)
										.charAt(2) ) {
							buttonList.get(k).setDisable(true);
							buttonList.get(k).setStyle("-fx-background-color: green");
						}
	...
	private void setDragLengthOne() {
		...
		for (Node target : SplayerGrid.getChildren()) {
	...
	private void configureDroppedImageOne() {
		...
		ObservableList<Node> buttonList = SplayerGrid.getChildren();
...
//							Button targetButton = (Button) target;
							if (orientation == "Horizontal") {
								...
								if (target.getId().toString().length() == 3) {
									...
									case 'A':
										twoController.addTargetToArmada((Button)target, serverArmada);
										for (int i = 0; i < 100; i++) {
											...
											if (button.getId().toString().length() == 3) {
												if (twoController.highlightLengthTwoHorizontalTargetThree(button, (Button) target, 'B'))
													twoController.addButtonsToArmada(button, serverArmada);
									...
									switch (target.getId().toString().charAt(1)) {
									case 'A':
										twoController.addTargetToArmadaFour((Button)target, serverArmada);
										for (int i = 0; i < 100; i++) {
											Node button = buttonList.get(i);
											if (button.getId().toString().length() == 4) {
												if (twoController.highlightLengthTwoHorizontalTargetFour(button, (Button) target, 'B'))
													twoController.addButtonsToArmadaFour(button, serverArmada);
											}
...
//									System.out.println(target.getId());
									...
									if (target.getId().toString().length() == 3) {
										switch (target.getId().toString().charAt(1)) {
										case 'A':
											threeController.addTargetToSubmarine((Button) target, serverArmada);
											for (int i = 0; i < 100; i++) {
												Node button = buttonList.get(i);
												if (button.getId().toString().length() == 3) {
													if (threeController.highlightLengthThreeHorizontalTargetThree(button, (Button)target, 'B', 'C'))
														threeController.addButtonsToSubmarine(button, serverArmada);
												}
											}
											break;
										...
										switch (target.getId().toString().charAt(1)) {
										case 'A':
											threeController.addTargetToSubmarineFour((Button) target, serverArmada);
											for (int i = 0; i < 100; i++) {
												Node button = buttonList.get(i);
												if (button.getId().toString().length() == 4) {
													if (threeController.highlightLengthThreeHorizontalTargetFour(button, (Button)target, 'B', 'C'))
														threeController.addButtonsToSubmarineFour(button, serverArmada);
												}
							...
							if (orientation == "Horizontal") {
								if (event.getGestureSource() != target && event.getDragboard().hasString()) {
									if (target.getId().toString().length() == 3) {
										switch (target.getId().toString().charAt(1)) {
										case 'A':
											threeController.addTargetToCruiser((Button) target, serverArmada);
											for (int i = 0; i < 100; i++) {
												Node button = buttonList.get(i);
												if (button.getId().toString().length() == 3) {
													if (threeController.highlightLengthThreeHorizontalTargetThree(button, (Button) target, 'B', 'C'))
														threeController.addButtonsToCruiser(button, serverArmada);
										...
										switch (target.getId().toString().charAt(1)) {
										case 'A':
											threeController.addTargetToCruiserFour((Button) target, serverArmada);
											serverArmada.addToCruiser(s.toString());
											for (int i = 0; i < 100; i++) {
												Node button = buttonList.get(i);
												if (button.getId().toString().length() == 4) {
													if (threeController.highlightLengthThreeHorizontalTargetFour(button, (Button) target, 'B', 'C'))
														threeController.addButtonsToCruiserFour(button, serverArmada);
												}
						...
						if (db.hasString()) {
							...
							if (orientation == "Horizontal") {
								...
								if (target.getId().toString().length() == 3) {
									switch (target.getId().toString().charAt(1)) {
									case 'A':
										fourController.addTargetToBattleship((Button) target, serverArmada);
										for (int i = 0; i < 100; i++) {
											Node button = buttonList.get(i);
											if (button.getId().toString().length() == 3) {
												if (fourController.highlightLengthFourHorizontalTargetThree(button, (Button) target, 'B', 'C', 'D'))
													fourController.addButtonToBattleship(button, serverArmada);
											}
									...
									switch (target.getId().toString().charAt(1)) {
									case 'A':
										fourController.addTargetToBattleshipFour((Button) target, serverArmada);
										for (int i = 0; i < 100; i++) {
											Node button = buttonList.get(i);
											if (button.getId().toString().length() == 4) {
												if (fourController.highlightLengthFourHorizontalTargetFour(button, (Button) target, 'B', 'C', 'D'))
													fourController.addButtonToBattleshipFour(button, serverArmada);
											}
...
//									System.out.println(target.getId());
									...
									if (target.getId().toString().length() == 3) {
										switch (target.getId().toString().charAt(1)) {
										case 'A':
											fiveController.addTargetToCarrier((Button) target, serverArmada);
											for (int i = 0; i < 100; i++) {
												Node button = buttonList.get(i);
												if (button.getId().toString().length() == 3) {
													if (fiveController.highlightLengthFiveHorizontalTargetThree(button, (Button) target, 'B', 'C', 'D', 'E'))
														fiveController.addButtonsToCarrier(button, serverArmada);
										...
										switch (target.getId().toString().charAt(1)) {
										case 'A':
											fiveController.addTargetToCarrierFour((Button) target, serverArmada);
											for (int i = 0; i < 100; i++) {
												Node button = buttonList.get(i);
												if (button.getId().toString().length() == 4) {
													if (fiveController.highlightLengthFiveHorizontalTargetFour(button, (Button) target, 'B', 'C', 'D', 'E'))
														fiveController.addButtonsToCarrierFour(button, serverArmada);
												}
